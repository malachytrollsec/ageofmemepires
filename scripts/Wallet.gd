...
func connect_wallet() -> void:
	...
	(function(){
		window.__memepireWalletAddress = window.__memepireWalletAddress || "";
		window.__memepireWalletToken = "";
		window.__memepireWalletVerified = false;
		window.__memepireWalletError = "";
		if (window.solana && window.solana.isPhantom) {
			window.solana.connect().then(async function(r){
				const address = r.publicKey.toString();
				window.__memepireWalletAddress = address;
				...
				window.__memepireWalletToken = login.token || '';
				window.__memepireWalletVerified = !!login.verified;
			}).catch(function(err){
				window.__memepireWalletError = String(err && err.message || err);
			});
			return "";
		}
		window.__memepireWalletAddress = "DEMO" + Math.random().toString(36).slice(2, 10).toUpperCase();
		return window.__memepireWalletAddress;
	})()
	""", true)
...
func _read_web_wallet_state() -> bool:
	...
	var a = JavaScriptBridge.eval("window.__memepireWalletAddress || ''", true)
	var t = JavaScriptBridge.eval("window.__memepireWalletToken || ''", true)
	var v = JavaScriptBridge.eval("!!window.__memepireWalletVerified", true)
	var e = JavaScriptBridge.eval("window.__memepireWalletError || ''", true)
...
func disconnect_wallet() -> void:
	...
	if OS.has_feature("web"):
		JavaScriptBridge.eval("window.__memepireWalletAddress=''; window.__memepireWalletToken=''; window.__memepireWalletVerified=false; window.__memepireWalletError='';", true)
	changed.emit()
