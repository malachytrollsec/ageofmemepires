...
func _ready() -> void:
	...
	# Compact scores panel; keep it plain so the text owns the box, not the frame art.
	leaderboard_panel = PanelContainer.new()
	leaderboard_panel.add_theme_stylebox_override("panel", _flat_menu_panel())
	leaderboard_panel.set_anchors_preset(Control.PRESET_TOP_LEFT)
	leaderboard_panel.offset_left = 16
	leaderboard_panel.offset_right = 316
	leaderboard_panel.offset_top = 16
	leaderboard_panel.custom_minimum_size = Vector2(292, 0)
	add_child(leaderboard_panel)
	var lbvb := VBoxContainer.new()
	lbvb.add_theme_constant_override("separation", 3)
	leaderboard_panel.add_child(lbvb)
	var lbh := _label("GAME SCORES", f_ui, 10, Game.COL_ACCENT_BRIGHT)
	lbh.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
	lbvb.add_child(lbh)
	leaderboard_status_lbl = _label("LOCAL SCORES", f_ui, 8, Game.COL_MUTED)
...
func _refresh_leaderboard_panel(entries := [], remote := false) -> void:
	...
	if is_instance_valid(leaderboard_status_lbl):
		leaderboard_status_lbl.text = "WEB SCORES" if remote else "LOCAL SCORES"
	var printed := 0
	if remote:
		for item in entries:
	...
	if printed == 0:
		if remote and is_instance_valid(leaderboard_status_lbl):
			leaderboard_status_lbl.text = "WEB SCORES EMPTY"
		for k in Game.KINGS.keys():
			var rec: Dictionary = Game.board.get(k, {"wins": 0, "losses": 0})
			var row2 := _label("%-7s %2dW %2dL" % [String(Game.KINGS[k]["name"]).to_upper().left(7), int(rec["wins"]), int(rec["losses"])], f_ui, 9, Game.KINGS[k]["color"])
...
func _leaderboard_web_row(item: Dictionary, rank: int) -> Label:
	...
	var row := _label("#%d %-7s %s %5d" % [rank, name.to_upper().left(7), result, score], f_ui, 9, col)
...
func _request_web_leaderboard() -> void:
	...
	if is_instance_valid(leaderboard_status_lbl):
		leaderboard_status_lbl.text = "SYNCING SCORES"
	JavaScriptBridge.eval("""
(function(){
  window.__avwLeaderboardStatus = 'loading';
  fetch('/leaderboard', {cache: 'no-store'}).then(function(res){ return res.json(); }).then(function(data){
    window.__avwLeaderboard = data || {};
    window.__avwLeaderboardStatus = 'ready';
    window.__avwState = Object.assign(window.__avwState || {}, {
      leaderboardLoaded: true,
      leaderboardRows: ((data && data.entries) || []).length
    });
  }).catch(function(err){
    window.__avwLeaderboardStatus = 'error';
    window.__avwLeaderboardError = String(err && err.message || err);
  });
  return true;
})()
	""", true)
...
func _poll_web_leaderboard(sync_id: int, attempt: int) -> void:
	...
	if attempt < 16:
		...
		leaderboard_status_lbl.text = "WEB SCORES ERROR" if status == "error" else "LOCAL SCORES"
...
func _read_web_leaderboard() -> bool:
	...
	if _web_leaderboard_status() == "error":
		if is_instance_valid(leaderboard_status_lbl):
			leaderboard_status_lbl.text = "WEB SCORES ERROR"
			leaderboard_status_lbl.add_theme_color_override("font_color", Game.COL_ENEMY)
		return false
	var raw = JavaScriptBridge.eval("JSON.stringify(window.__avwLeaderboard || {})", true)
	if raw == null:
		return false
	var text := String(raw)
	if text == "" or text == "{}":
...
func _web_leaderboard_status() -> String:
	if not OS.has_feature("web"):
		return ""
	var status = JavaScriptBridge.eval("window.__avwLeaderboardStatus || ''", true)
...
func _refresh_launch_status() -> void:
	...
	if not is_instance_valid(launch_status_lbl):
		return
	var mode := "VERIFIED WALLET" if Wallet.verified else "UNSIGNED MODE"
	var launch_state := "READY"
	if Net.connected:
...
func _room_links_text() -> String:
	...
	return "Ansem vs Wynn room %s\nHost: %s/?room=%s&host=1&king=%s&rival=%s\nJoin: %s/?room=%s&join=1&king=%s&rival=%s\nRoom Kit: %s/room-kit?room=%s&player=%s&rival=%s" % [
		room,
		origin, room.uri_encode(), selected_king.uri_encode(), rival.uri_encode(),
		origin, room.uri_encode(), rival.uri_encode(), selected_king.uri_encode(),
		origin, room.uri_encode(), selected_king.uri_encode(), rival.uri_encode(),
	]

func _room_links_smoke() -> void:
	selected_king = "ansem"
	Game.rival_king = "wynn"
	room_edit.text = "AVW"
	var links := _room_links_text()
	var host_ok := links.contains("Host: http://127.0.0.1:8799/?room=AVW&host=1&king=ansem&rival=wynn")
	var join_ok := links.contains("Join: http://127.0.0.1:8799/?room=AVW&join=1&king=wynn&rival=ansem")
...
func _refresh_room_share() -> void:
	if room_share_lbl:
		room_share_lbl.text = "share code %s | launch room ready" % _room_code()

func _on_room_message(_msg: Dictionary) -> void:
	return
...
func _on_launch_match(cfg: Dictionary) -> void:
	...
	var host_king := String(cfg.get("host_king", "ansem"))
	if Net.role == "join":
...
func _card(king: String) -> Button:
	...
	var b := Button.new()
	b.custom_minimum_size = Vector2(190, 194)
	b.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
