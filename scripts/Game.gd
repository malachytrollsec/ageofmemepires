...
# --- Launch factions ---
const KINGS := {
	"ansem": {"name": "ANSEM", "kingdom": "Alien Tape", "color": Color("7fe8ff"), "asset": "ansem"},
	"wynn": {"name": "WYNN", "kingdom": "Predator Bid", "color": Color("c6ff5f"), "asset": "wynn"},
}
...
const KING_BONUS := {
	"ansem": {"gather": 1.0, "hp": 1.18, "speed": 1.0, "atk": 1.05},
	"wynn": {"gather": 1.12, "hp": 1.0, "speed": 1.12, "atk": 1.0},
}
...
func publish_web_state(patch: Dictionary) -> void:
	if not OS.has_feature("web"):
		return
	var js := "window.__avwState = Object.assign(window.__avwState || {}, %s);" % JSON.stringify(patch)
...
func web_event(name: String, data := {}) -> void:
	...
	var js := "window.__avwEvents = window.__avwEvents || []; window.__avwEvents.push(%s); window.__avwEvents = window.__avwEvents.slice(-80);" % JSON.stringify(payload)
	JavaScriptBridge.eval(js, true)
...
func _post_web_result(king: String, won: bool, meta: Dictionary) -> void:
	...
	var entry := {
		...
		"walletLabel": Wallet.short() if Wallet.verified else "Unsigned",
...
fetch('/leaderboard', {
  ...
  window.__avwState = Object.assign(window.__avwState || {}, {leaderboardPosted: false, leaderboardError: String(err && err.message || err)});
});
