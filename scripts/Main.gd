...
const BATTLEFIELD_SAND_PATH := "res://assets/terrain/terrain_sand_tile.png"
...
func _ready() -> void:
	...
	Game.log_event("Skirmish live. Scores enabled.")
	...
	if _truthy(_web_query_params().get("unitSpritePreview", "")):
		call_deferred("_web_unit_sprite_preview")
...
func _web_unit_sprite_preview() -> void:
	if not OS.has_feature("web"):
		return
	await get_tree().process_frame
	Game.player_king = "ansem"
	...
	for u in units:
		if _node_alive(u):
			...
			u.selected = false
	var placements := [
		{"king": "ansem", "kind": "villager", "pos": player_keep.position + Vector2(80, -470), "dir": Vector2.DOWN},
		{"king": "ansem", "kind": "swordsman", "pos": player_keep.position + Vector2(142, -478), "dir": Vector2.RIGHT},
		{"king": "ansem", "kind": "archer", "pos": player_keep.position + Vector2(204, -486), "dir": Vector2.UP},
		{"king": "ansem", "kind": "lancer", "pos": player_keep.position + Vector2(284, -500), "dir": Vector2.RIGHT},
		{"king": "ansem", "kind": "siege", "pos": player_keep.position + Vector2(382, -508), "dir": Vector2.LEFT},
		{"king": "wynn", "kind": "villager", "pos": player_keep.position + Vector2(80, -352), "dir": Vector2.DOWN},
		{"king": "wynn", "kind": "swordsman", "pos": player_keep.position + Vector2(142, -360), "dir": Vector2.RIGHT},
		{"king": "wynn", "kind": "archer", "pos": player_keep.position + Vector2(204, -368), "dir": Vector2.UP},
		{"king": "wynn", "kind": "lancer", "pos": player_keep.position + Vector2(292, -382), "dir": Vector2.RIGHT},
		{"king": "wynn", "kind": "siege", "pos": player_keep.position + Vector2(392, -390), "dir": Vector2.LEFT},
	]
	var kinds := []
	var kings := []
	for p in placements:
	...
	_publish_web_match_state({
		"unitSpritePreview": true,
		"unitSpriteKings": kings,
		"unitSpriteKinds": kinds,
		"unitSpriteUnits": placements.size(),
	})
...
func _remote_result_smoke() -> void:
	...
	var before_wins := int(Game.board.get("wynn", {}).get("wins", 0))
	_apply_snapshot({
		"over": true,
		"result": "lost",
		"seconds": 37,
	...
	await get_tree().create_timer(0.2).timeout
	var after_wins := int(Game.board.get("wynn", {}).get("wins", 0))
...
func _show_banner(text: String, col: Color) -> void:
	...
	result.text = "RESULT RECORDED"
	result.add_theme_font_override("font", load("res://assets/fonts/silkscreen.ttf"))
	result.add_theme_font_size_override("font_size", 14)
	result.add_theme_color_override("font_color", Game.COL_BONE)
...
func _apply_snapshot(snap: Dictionary) -> void:
	...
	for d in snap.get("units", []):
		...
		if not _node_alive(u):
			u = _spawn_unit(String(d.get("king", "ansem")), String(d.get("kind", "villager")), int(d.get("team", 0)), Vector2(float(d.get("x", 0)), float(d.get("y", 0))), id, false)
...
func _draw() -> void:
	...
	if battlefield_texture:
		_draw_tiled_ground(battlefield_texture, Color(1, 1, 1, 1))
...
func _draw_tiled_ground(tex: Texture2D, tint: Color) -> void:
	var tile_size := tex.get_size()
	if tile_size.x <= 0.0 or tile_size.y <= 0.0:
		return
	for x in range(0, int(WORLD.x) + int(tile_size.x), int(tile_size.x)):
		for y in range(0, int(WORLD.y) + int(tile_size.y), int(tile_size.y)):
			draw_texture_rect(tex, Rect2(Vector2(x, y), tile_size), false, tint)
