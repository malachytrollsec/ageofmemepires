...
const root = process.env.AVW_WEB_ROOT || join(projectRoot, "build/web");
...
const dataDir = process.env.AVW_DATA_DIR || join(projectRoot, ".avw-data");
const leaderboardFile = join(dataDir, "leaderboard.json");
const legacyLeaderboardFile = join(projectRoot, ".avw-runtime/leaderboard.json");
const leaderboardLimit = 25;
const leaderboardStorageLimit = 80;
const factionsManifestPaths = new Set(["/avw-factions.json", "/.well-known/avw-factions.json"]);
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/room-kit") {
      ...
      const player = sanitizeKing(url.searchParams.get("player") || url.searchParams.get("king") || "ansem");
      const rival = sanitizeKing(url.searchParams.get("rival") || "wynn");
      return json(res, {
        room,
        game: "Ansem vs Wynn",
        brand: brandPayload(),
        ...
        avwFactionsUrl: `${originUrl(req)}/avw-factions.json?room=${encodeURIComponent(room)}`,
    ...
    if (p.endsWith(".html")) {
      ...
      body = Buffer.from(String(body).replace("</head>", `<script>window.AVW_ROOM_RELAY=${JSON.stringify(relay)};</script></head>`));
    }
    res.writeHead(200, {
      ...headers,
    });
...
server.listen(port, "127.0.0.1", () => console.log(`avw web build + room relay on ${port}`));
...
function roomProofPayload(filterCode = "") {
  ...
  }).filter((room) => filterCode || room.peerCount > 0 || room.snapshotCount > 0 || room.intentCount > 0);
...
function verifiedLeaderboardPayload(req) {
  return {
    version: "avw-verified-leaderboard-v1",
    leaderboardUrl: `${originUrl(req)}/leaderboard`,
    challengeUrl: `${originUrl(req)}/wallet-challenge`,
    loginUrl: `${originUrl(req)}/wallet-login`,
    walletFlag: "--wallet",
    walletTokenFlag: "--wallet-token",
    walletEnv: "AVW_KING_WALLET",
    walletTokenEnv: "AVW_KING_WALLET_TOKEN",
    tokenField: "walletToken",
    submit: {
      method: "POST",
...
function kingsPayload(req) {
  return ["ansem", "wynn"].map((id) => kingProfile(id, req));
}

function kingProfile(id, req) {
  const clean = sanitizeKing(id);
  const labels = {
    ansem: ["ANSEM", "Alien Tape"],
    wynn: ["WYNN", "Predator Bid"],
  };
  const [name, kingdom] = labels[clean] || labels.ansem;
  const assetId = clean === "wynn" ? "wynn" : "ansem";
  ...
  return {
    ...
    units: unitKinds,
    bonus: {
      ansem: "alien pressure and tougher frontline trades",
      wynn: "hunter speed and sharper market rotations",
    }[clean],
  };
}
...
function sanitizeKing(value) {
  ...
  if (["ansem", "alien", "blknoiz06"].includes(id)) return "ansem";
  if (["wynn", "predator", "jameswynnreal"].includes(id)) return "wynn";
  return ["ansem", "wynn"].includes(id) ? id : "ansem";
}
...
async function readLeaderboard() {
  for (const file of [leaderboardFile, legacyLeaderboardFile]) {
    try {
      ...
      if (error?.code && error.code !== "ENOENT") console.warn(`[avw] leaderboard read skipped: ${error.message}`);
    }
  }
  return [];
}
...
function normalizeLeaderboardEntry(item = {}, walletAuth = null) {
  ...
  const score = clampInt(item.score || leaderboardScore({ result, wave, kills, seconds }), 0, 9999999);
  const endedAt = String(item.endedAt || new Date().toISOString()).slice(0, 40);
  const kingId = cleanId(item.kingId || item.king || "ansem", "ansem");
  const rivalId = cleanId(item.rivalId || item.rivalKing || "wynn", "wynn");
...
function leaderboardScore({ result, wave, kills, seconds }) {
  const finish = result === "won" ? 1000 : 120;
  const timePenalty = Math.floor(seconds / 12);
  return Math.max(1, finish + wave * 80 + kills * 35 - timePenalty);
}
